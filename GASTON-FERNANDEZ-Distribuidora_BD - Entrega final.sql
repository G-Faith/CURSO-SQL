...
-- Creo tablas.
	-- MERCADO ----------------------------------------------------------------
       ...
       CREATE TABLE IF NOT EXISTS Distribuidora.Market
       (
       ID_MARKET INT NOT NULL AUTO_INCREMENT primary KEY, -- > Identificador unico deL mercado.
       Market VARCHAR(20) NOT NULL, -- > Nombre del mercado.
       Category VARCHAR(20) NOT NULL  -- > Categoria de mercado.
       );
	...
	-- LOCACIÓN ----------------------------------------------------------------
       ...
       CREATE TABLE IF NOT EXISTS Distribuidora.location
       (
       ID_LOCATION INT NOT NULL AUTO_INCREMENT primary KEY, -- > Identificador unico de la locación.
       Location VARCHAR(20) NOT NULL, -- > Nombre de la provincía.
       Category VARCHAR(20) NOT NULL  -- > Categoria de la província.
       );
	...
	-- CLIENTES -----------------------------------------------------------------------------------
		...
		(
            ...
            ID_LOCATION INT NOT NULL, -- > clave foranea de locación del cliente
            ID_MARKET INT NOT NULL, -- > clave foranea de mercado de pertenencia.
            -- Creo relaciones
   ...
   -- COMPRA -----------------------------------------------------------------------------------
	...
	-- RECLAMOS ----------------------------------------------------------------
       ...
       CREATE TABLE IF NOT EXISTS Distribuidora.claim
       (
       ID_CLAIM INT NOT NULL AUTO_INCREMENT primary KEY, -- > Identificador unico de la locación.
       claim_date DATE NOT NULL, -- > fecha de reclamo.
       claim_Description TEXT NOT NULL,  -- > Detalle del reclamo.
       -- Creo las claves foraneas.
       ID_SELL INT NOT NULL, -- Identificador de venta.
       -- Creo las relaciones.
       FOREIGN KEY (ID_SELL) REFERENCES SELL(ID_SELL)
       );
	...
	-- DETALLE DE RECLAMOS ----------------------------------------------------------------
       ...
       CREATE TABLE IF NOT EXISTS Distribuidora.Det_claim
       (
       ID_DET_CLAIM INT NOT NULL AUTO_INCREMENT primary KEY, -- > Identificador unico de la locación.
       quantity INT NOT NULL,  -- > Cantidades reclamadas.
       Find VARCHAR(20) NOT NULL, -- > Categoria del hallazgo
       -- Creo las claves foraneas.
       ID_CLAIM INT NOT NULL, -- Identificadotr de reclamo.
       ID_PRODUCT INT NOT NULL, -- Identificador de producto.
       -- Creo las relaciones.
       FOREIGN KEY (ID_PRODUCT) REFERENCES PRODUCT(ID_PRODUCT),
       FOREIGN KEY (ID_CLAIM) REFERENCES CLAIM(ID_CLAIM)
       );
...
-- Step 1: Inserción de datos.
	-- Inserto datos tabla de mercado.
INSERT INTO Market (Market, Category) VALUES
('Dietética','Pequeño'),
('Dietética','Mediano'),
('Dietética','Grande'),
('Retail','Pequeño'),
...
('Cadena','Corporativo');
	...
	-- Inserto datos tabla de locación.
INSERT INTO location (Location, Category) VALUES
('Buenos Aires','AMBA'),
('CABA','CABA'),
('Córdoba','Interior'),
('Santa Fe','Interior'),
...
INSERT INTO CLIENT 
(Company_name, fisrt_name_contact, last_name_contact, Address, Email, CUIT, ID_LOCATION, ID_MARKET)
VALUES
('Dietética Vida Sana','Juan','Perez','Av Corrientes 1234','dieteticavidasana@gmail.com','30-12345678-9',2,1),
('Alimentos Naturales SRL','María','Gomez','Av Santa Fe 2210','alimentosnaturales@gmail.com','30-23456789-1',1,8),
('Mercado Saludable','Carlos','Lopez','Av Cabildo 876','mercadosaludable@gmail.com','30-34567891-2',1,4),
('Tienda Sin Gluten','Ana','Martinez','Av Rivadavia 3321','tiendasingluten@gmail.com','30-45678912-3',2,2),
('Distribuidora Natural','Pedro','Sanchez','Belgrano 120','distribuidoranatural@gmail.com','30-56789123-4',3,9),
('Vida Fit','Lucia','Ramirez','Colon 800','vidafit@gmail.com','30-67891234-5',3,1),
('Almacen Orgánico','Diego','Fernandez','San Martin 500','almacenorganico@gmail.com','30-78912345-6',5,5),
('Salud Integral','Sofia','Diaz','Mitre 900','saludintegral@gmail.com','30-89123456-7',4,2),
('Green Market','Martin','Torres','9 de Julio 333','greenmarket@gmail.com','30-91234567-8',6,4),
('NutriVida','Paula','Suarez','Laprida 450','nutrivida@gmail.com','30-22334455-6',9,1),
('Eco Alimentos','Ricardo','Castro','Sarmiento 222','ecoalimentos@gmail.com','30-33445566-7',11,7),
('Bio Market','Carla','Rojas','Italia 333','biomarket@gmail.com','30-44556677-8',4,5),
('Natural Store','Fernando','Acosta','Urquiza 900','naturalstore@gmail.com','30-55667788-9',11,2),
('Saludable Market','Elena','Molina','España 222','saludablemarket@gmail.com','30-66778899-0',7,4),
('Dietetica Centro','Pablo','Navarro','Av Alem 120','dieteticacentro@gmail.com','30-77889911-2',8,3);	
...
INSERT INTO claim (claim_date, claim_Description, ID_SELL) VALUES
('2025-02-05','Producto roto en transporte',1),
('2025-02-06','Faltante de unidades en pedido',2),
('2025-02-07','Producto vencido',3),
('2025-02-08','Error en producto enviado',4),
('2025-02-09','Cantidad incorrecta',5),
('2025-02-10','Envase dañado',6),
('2025-02-11','Producto aplastado',7),
('2025-02-12','Error de picking',8),
('2025-02-13','Producto incorrecto',9),
('2025-02-14','Faltante parcial',10),
('2025-02-15','Producto vencido',11),
('2025-02-16','Diferencia en factura',12),
('2025-02-17','Producto en mal estado',13),
('2025-02-18','Error logístico',14),
('2025-02-19','Producto equivocado',15);

	-- Inserto datos de reclamos.
INSERT INTO Det_claim (quantity, Find, ID_CLAIM, ID_PRODUCT) VALUES
(2,'Rotura',1,1),
(5,'Faltante',2,2),
(3,'Vencido',3,3),
(4,'Error picking',4,4),
(6,'Cantidad incorrecta',5,5),
(2,'Envase dañado',6,6),
(1,'Rotura',7,7),
(3,'Error picking',8,8),
(2,'Producto incorrecto',9,9),
(5,'Faltante',10,10),
(4,'Vencido',11,11),
(2,'Error facturación',12,12),
(3,'Mal estado',13,13),
(1,'Error logístico',14,14),
(2,'Producto incorrecto',15,15);
...
-- Vista 4
    ...
    CREATE OR REPLACE VIEW vw_provincia_clave AS
SELECT
    pv.location AS Provincia,
    ROUND(SUM(ds.quantity * p.unit_x_pack * p.unit_price)) AS facturado
FROM sell s
JOIN client c ON c.ID_Client = s.ID_Client
JOIN det_sell ds ON ds.ID_Sell = s.ID_Sell
JOIN product p ON p.ID_Product = ds.ID_Product
JOIN location pv ON pv.ID_LOCATION= c.ID_LOCATION
GROUP BY pv.location
ORDER BY Facturado DESC LIMIT 5;

-- Vista 5
	-- > Producto critico
    
    CREATE OR REPLACE VIEW vw_producto_critico AS
SELECT
    p.name AS Producto,
    ROUND(SUM(dc.quantity * p.unit_x_pack)) AS UNReclamada
FROM product as p
JOIN det_claim dc ON dc.ID_PRODUCT = p.ID_PRODUCT
GROUP BY p.name
ORDER BY UNReclamada DESC LIMIT 5; 
